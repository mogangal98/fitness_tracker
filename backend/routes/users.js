...
router.get("/me", authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id, name, email, equipment, role, last_advice_at, height_cm, weight_kg, created_at FROM users WHERE id = $1",
      [req.user.id]
    );
...
// PUT /api/users/me/metrics — update height and weight
router.put("/me/metrics", authMiddleware, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE users SET height_cm = COALESCE($1, height_cm), weight_kg = COALESCE($2, weight_kg) WHERE id = $3 RETURNING id, name, email, equipment, height_cm, weight_kg",
      [h, w, req.user.id]
    );
